...
def article_from_servicenow(record: dict) -> Article:
    ...
    future kb_knowledge reader must use: configured metadata column names
    map back to the canonical fields used by indexing.
    """

    metadata = {
        source_field: _value(record.get(servicenow_field, ""))
        for source_field, servicenow_field
        in SERVICENOW.kb_metadata_field_map.items()
    }

    article_number = (
        metadata.get("article_number")
        or _value(record.get("number", ""))
    )
...
def load_articles_from_servicenow() -> list[Article]:
    ...
    return [
        article_from_servicenow(record)
